...
import { chat as aiChat, summarize as aiSummarize } from '../../lib/aiRouter';
import { getOrCreateSession, saveConversation } from '../../lib/chatApi';
import { buildIndex, getRAGContext } from '../../lib/rag/retriever';
...
export default function AIPanel() {
  ...
  const { getActiveNote, noteAICache, setAICache, notes } = useNoteStore();

  // Build RAG index on mount
  useEffect(() => {
    if (notes.length > 0) buildIndex(notes);
  }, [notes]);
  ...
  const sendChatMessage = async () => {
    ...
    try {
      ...
      const ragContext = getRAGContext(text);
      const context = noteContext + (ragContext ? '\n\nRelevant notes:\n' + ragContext : '');
      const history = chatMessages.map(m => ({
        role: m.role === 'ai' ? 'assistant' : 'user',
        content: m.text,
      }));

      console.log('[AIPanel] Sending chat...', { textLen: text.length, contextLen: context.length, historyLen: history.length });

      setChatMessages(prev => [...prev, { role: 'ai', text: '' }]);

      await aiChat(text, context, history, (chunk, fullText) => {
      ...
      console.error('[AIPanel] Chat error:', error);
  ...
  const handleSaveChat = async () => {
    console.log('[AIPanel] Save clicked', { noteId: activeNote?.id, msgCount: chatMessages.length });
    ...
    try {
      const userId = 'local';
      const session = await getOrCreateSession(userId, activeNote.id, activeNote.title);
      console.log('[AIPanel] Session:', session);
      const msgs = chatMessages
        .filter(m => m.text)
        .map(m => ({ role: m.role === 'ai' ? 'assistant' : 'user', text: m.text }));
      console.log('[AIPanel] Saving msgs:', msgs.length);
      await saveConversation(session.id, msgs);
      console.log('[AIPanel] Save complete');
      setSaveSuccess(true);
  ...
  const generateSummary = async () => {
    ...
    try {
      ...
      let summaryResult = '';
      await aiSummarize(plainText, (chunk, fullText) => {
        summaryResult = fullText;
      });
      if (summaryResult && activeNote) setAICache(activeNote.id, 'summary', summaryResult);
    } catch (err) {
  ...
  // ── TTS (disabled in local mode — requires server-side TTS service) ──
  const generateTTS = async () => {
    setTtsLoading(true);
    // TTS requires a server-side service. In local mode, this feature is unavailable.
    setTimeout(() => setTtsLoading(false), 300);
  };
